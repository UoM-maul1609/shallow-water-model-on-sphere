...
for it1 in range(3,len(time)+1,4):
    ...
    cs1 = map.pcolor(x,y,h[it,:,:],cmap='jet', shading='auto', vmin=59000, vmax=64000)
    ...
    cbar.set_label('h, m')
    ax1.set_title('Height at \n t=' + '{0:.2f}'.format(time[it]/(86400)) +' days')
    ...
    # contour data over the map.
    #cs = map.contour(x,y,wave+mean,15,linewidths=1.5)
    cs2 = map.pcolor(x,y,vort[it,:,:],cmap='jet',shading='auto', vmin=-0.00008, vmax=0.00008)
    cbar=map.colorbar(location='bottom')
    cbar.ax.set_xticks(cbar.ax.get_xticks())
    cbar.ax.set_xticklabels(cbar.ax.get_xticklabels(), rotation='vertical')
    ...
    # contour data over the map.
    #cs = map.contour(x,y,wave+mean,15,linewidths=1.5)
    cs3 = map.pcolor(x,y,v[it,:,:],cmap='jet',shading='auto', vmin=-7, vmax=7)
    cbar=map.colorbar(location='bottom')
    ...
    # contour data over the map.
    #cs = map.contour(x,y,wave+mean,15,linewidths=1.5)
    cs4 = map.pcolor(x,y,u[it,:,:],cmap='jet',shading='auto', vmin=-5, vmax=60)
    cbar=map.colorbar(location='bottom')
