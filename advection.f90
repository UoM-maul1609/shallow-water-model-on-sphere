...
! 		enddo																				
    ...
    subroutine lax_wendroff_ll(ip,jp,o_halo,dt,g,u,v,h,hs,re,&
		...
		Vy2 = 0.5_sp*g*h**2
		vh_mid_xt(0:ip,1:jp) = 0.5_sp*(vh(1:ip+1,1:jp)+vh(0:ip,1:jp)) &
		  -(0.5_sp*dt/(recqdp_s(0:ip,1:jp)))*(Vx(1:ip+1,1:jp)-Vx(0:ip,1:jp)) &
		  -0.125_sp*dt*(f_cor(1:ip+1,1:jp)+f_cor(0:ip,1:jp))*(uh(1:ip+1,1:jp)+uh(0:ip,1:jp))
...
! 
    ...
    subroutine smagorinsky(ip,jp,o_halo,cvis,u,v,vis,re,&
		...
		implicit none
		integer(i4b), intent(in) :: ip,jp,o_halo
		real(sp), intent(in) :: cvis, re
		real(sp), intent(in), dimension(1-o_halo:ip+o_halo,1-o_halo:jp+o_halo) :: &
																		u,v, &
															recq, dp1, dq
		real(sp), intent(inout), dimension(1:ip,1:jp) :: vis
		! local variables:
		integer(i4b) :: j, i
		...
		vis(1:ip,1:jp) = cvis**2._sp*re*recq(1:ip,1:jp)*dp1(1:ip,1:jp)*dq(1:ip,1:jp)* &
		sqrt( ( (u(2:ip+1,1:jp)-u(0:ip-1,1:jp))/ &
			(recq(1:ip,1:jp)*(dp1(0:ip-1,1:jp)+dp1(1:ip,1:jp))) )**2._sp + &
			( (v(1:ip,2:jp+1)-v(1:ip,0:jp-1))/ &
			(re*(dq(1:ip,1:jp)+dq(1:ip,0:jp-1))) )**2._sp + &
			0.5_sp*(  &
